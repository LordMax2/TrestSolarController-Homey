...
class TrestSolarModule {
    async setEnergyMode(args, energyMode) {
        ...
        const formattedDateTime = new Date().toISOString().slice(0, 13) + ":00:00";
        const formattedDateTimeOneHourAhead = new Date(new Date().getTime() + 60 * 60 * 1000).toISOString().slice(0, 13) + ":00:00";

        const options = {
            hostname: solarUrl,
            port: solarPort,
            path: `/api/v1/solar/addbotactionoverrideduration?energyMode=${energyMode}&startAt=${formattedDateTime}&endAt=${formattedDateTimeOneHourAhead}`,
            method: 'POST',
            headers: {
                'X-Token': token
            },
